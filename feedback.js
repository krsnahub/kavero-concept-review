(() => {
  ...
  function review() {
    return { schemaVersion: 1, conceptRevision: 'ecb9c3e', catalogRevision: 'books-04-05-r1', reviewer: name.value.trim(), createdAt: new Date().toISOString(), stories: cards.map(card => ({id: card.dataset.storyId, title: card.querySelector('h3').textContent, ...state[card.dataset.storyId]})) };
  }
